...
class _DeviceConnectionScreenState extends State<DeviceConnectionScreen> {
  ...
  Future<void> _saveDevice(
    ...
    try {
      if ((protocol == 'wifi' || protocol == 'websocket') && host != null) {
        final isDuplicate = _savedDevices.any((device) =>
            (device.protocol == 'wifi' || device.protocol == 'websocket') && device.host == host);
        if (isDuplicate) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Perangkat Wi-Fi dengan alamat ini sudah tersimpan.'),
        ...
        if (isDuplicate) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Perangkat Bluetooth ini sudah tersimpan.'),
                backgroundColor: Colors.orange,
              ),
            );
          }
      ...
      builder: (context, _) {
        ...
        if (telloService.isConnected) {
          ...
          if (service.connectionMode == ConnectionMode.bluetooth) {
            activeDeviceName = service.selectedDevice?.name ?? 'Bluetooth Device';
          } else {
